...
#seperation audio channels     
def returntempname(filename):
    tempnames = ["data1.wav","data2.wav","data3.wav","data4.wav"]
    ...
    tempname1 = os.path.join(dirpath,tempnames[0])
    tempname2 = os.path.join(dirpath,tempnames[1])
    tempname3 = os.path.join(dirpath,tempnames[2])
    tempname4 = os.path.join(dirpath,tempnames[3])
    wavfile.write(tempname1,rate,data1)
    wavfile.write(tempname2,rate,data2)
    wavfile.write(tempname3,rate,data3)
    wavfile.write(tempname4,rate,data4)
    return tempname1

if __name__ == "__main__":
    restart = False
    rootpath=r"D:\bird classification\model"
    csvpath = r""#csv\mix.csv
    datapath = r""#overlapping_separation_audio_generator\three_birds\audios
    random.seed(7)
    torch.manual_seed(7)           
    ...
    model = xception()
    print("model done!")
    load_model = torch.load(os.path.join(rootpath,"save/bestxecption.pt"),map_location=torch.device('cpu'))
    ...
    #for each mutiple-channel overlapping files
    if restart == False:
        pre = []
        labels = []
        ...
        with open(r"./mix.json",'r') as f:
            save = json.load(f)
            pre = save["pre"]
            labels = save["labels"]
            lastindex = int(save["lastindex"])
    df = pd.read_csv(csvpath)
    df = df.iloc[lastindex+1:,:]
    for index,row in df.iterrows():
        print("index:",index)
        filepath = os.path.join(datapath,row["filepaths"])
        #label preparation 
        label = row["labels"].split("+")
        print(label)
        label = [CLASS[i] for i in label]
        aname1,aname2,aname3,aname4 = returntempname(filepath)
        tempkey =  np.zeros(50, dtype=float)
        p1,key1 = final(model = model,path = aname1,device = device)#预测临时文件
        tempkey = tempkey+p1    
        top3 = list(tempkey.argsort()[-3:][::-1])
        print(label)
        print(top3)
        label = list(map(float,label))
        top3 = list(map(float,top3))
        os.remove(aname1)  #del temp files
        os.remove(aname2) 
        os.remove(aname3)             
        os.remove(aname4) 
        classes = [4,5,9,19,49]
        ...
        for m in templ:
            labels.append(m)
        save["lastindex"]=str(index)
        save["pre"]=pre
        save["labels"]=labels
        print(save)
        with open(r'./mix.json', 'w') as f:
            json.dump(save, f)
